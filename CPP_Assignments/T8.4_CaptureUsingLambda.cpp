...
int main()
{
    ...
    // Lambda capturing by value (will cause error if you try to modify num)
    // auto lambda_by_value = [num]() { num = 20; }; // Uncommenting this line will cause a compilation error

    // Lambda capturing by reference (modification is allowed)
    auto lambda_by_ref = [&num]()
    {
        num = 20;
    ...
    lambda_by_ref();
    std::cout << "Outside lambda, num = " << num << std::endl;
